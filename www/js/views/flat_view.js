 ...
 *
 */
function FlatView(doc, filename, prefix) {
	this._logger = null;
	this._xml = null;
...
FlatView.prototype._init = function(doc, filename, prefix) {
	this._doc = doc;
...
FlatView.prototype._log = function(data) {
	if (this._logger) this._logger.log(data);
};
FlatView.prototype._debug = function(data) {
	if (this._logger) this._logger.debug(data);
};

// RESOLVETHIS
FlatView.prototype._getContext = function(type, tag, model, blockid) {
	var obj;
...
FlatView.prototype.renderModel = function(schema, model) {
	if (! model) {
...
FlatView.prototype._renderModelWrapper = function() {
...
FlatView.prototype._topLevelCif = function(parentXmlNode) {
	...
	for (var i = 0; i < blocks.length; i++) {
		...
		if (type == 'global_' || type == 'data_') {
			...
			this._walk(block, progeny, childXmlNode);
...
FlatView.prototype._topLevelDic = function(parentXmlNode) {
	var blocks = this._model.getBlocks();
	...
	for (var i = 0; i < blocks.length; i++) {
		...
		if (type == 'global_' || type == 'data_') {
			...
			if (sundries) {
				// this doesn't include those mixed up in looped categories
				var dl = this._doc.createElement('div');
				dl.setAttribute('class','category_flat');
				childXmlNode.appendChild(dl);
				var dt = this._doc.createElement('div');
				dl.appendChild(dt);
				this._log("unresolved " + sundries.length);
				//this._walkGroupData(block, sundries, childXmlNode); 
				this._walkGroupData(block, sundries, dt); 
			}
		} 
...
FlatView.prototype._walk = function(block, siblings, parentXmlNode) {
	var cats = [];
...
FlatView.prototype._walkGroupData = function(block, leaves, parentXmlNode) {
...
FlatView.prototype._renderHeader = function(block, parentXmlNode) {
...
//	a.setAttribute('onclick',"CifJs.displayStateFlip('" + strn + "');event.returnValue=false;return false;");
	var dd = this._doc.createElement('div');
...
FlatView.prototype._renderBlock = function(block, parentXmlNode) {
	// otherwise it is a newBlock
	var type = block.getType();
...
//	text = this._doc.createTextNode("\240");
//	dt.appendChild(text);
	text = this._doc.createTextNode("data_" + strn);
	dt.appendChild(text);
...
//	dd.style.display = "none";
	...
	/*
      ...
      var func = function() {
        ...
        //tooltip.on("xxx"); 
        tip.onmouseout =  function() { tooltip.off(); };
        tip.onmouseover =  function() { tooltip.on(null);};
      }
  ...
  a.onmouseover =  function(evt) { 
     ...
     tooltip.move(evt);
         tooltip.timeout = setTimeout( function() {
             func(); tooltip.on(evt);tooltip.move(evt); }, 500);
  };
  a.onmousemove =  function(event) { tooltip.move(event); };
...
FlatView.prototype._renderCategory = function(block, cat_name, parentXmlNode) {
...
//	dd.style.display = "none";
    ...
    var func = function() {
      ...
      // a closure exploiting local var func
    a.onmouseover =  function(evt) { 
         tooltip.clearTimeout();
         tooltip.move(evt);
         tooltip.timeout = setTimeout( function() {
...
FlatView.prototype._renderAttribute = function(frag, parentXmlNode, blockid){
	var chunk = frag.getJSON();
	var dl = this._doc.createElement('div');
	dl.setAttribute('class','datum');
	var dt = this._doc.createElement('div');
	dl.appendChild(dt);
	var span = this._doc.createElement('span');
	dt.setAttribute('class','datum_name');
	var field = this._doc.createElement('a');
	var tag = chunk[1].toLowerCase();
	if (this._modelType == 'cif') {
		span.setAttribute('id', this._prefix + blockid + tag);
	}

	var path = null;
	if (this._schema) {
		path = this._schema.getItemPath(tag); 
	}
	if (path) {
		// href.nodeValue = "#dic:" + path[0];
		var prefix = this._schema.getItemDictPrefix(tag); 
...
//		field.setAttribute('onclick',"CifJs.openPath(function() {return CifJs._meta_dict.getItemPath('" + tag.toLowerCase() + "');},'" + prefix + "')");
		...
		dt.setAttribute('class','unknown_datum_name');
	}
	...
	var dd = this._doc.createElement('div');
	dd.setAttribute('class','datum_value');
	var txt = "?";
	if (chunk[2]) {
...
FlatView.prototype._buildEvalControls = function(tag, dt, frag, blockid, mode) {
...
FlatView.prototype._textExtents = function(txt) {
...
FlatView.prototype._renderComment = function(frag, parentXmlNode) {
	var chunk = frag._data;
	var div = this._doc.createElement('div');
...
FlatView.prototype._renderLoop = function(frag, parentXmlNode, blockid) {
	var chunk = frag.getJSON();
	...
	for (var k = 0; k < chunk[1].length; k++) { 
		...
		if (typeof tag == 'string') {
			...
			var field = this._doc.createElement('span');
			var path;
			if (this._schema) {
...
FlatView.prototype.addLoopRow = function(frag, view, vals, klass) {
	var tbody = view.children[2].children[1]; // dl dt dd table 
...
FlatView.prototype.updateLoopValue = function(view, vptr, val, klass) {
	var tbody = view.children[2].children[1]; // dl dt dd table 
	var rows = tbody.children;
	var tdcnt = 0;
	var cell;
...
FlatView.prototype._renderTextValue = function(textarea, txt, klass) {
	var bounds = this._textExtents(txt);
...
FlatView.prototype._renderSave = function(frag, parentXmlNode) {
	var chunk = frag._data;
	var dl = this._doc.createElement('dl');
	parentXmlNode.appendChild(dl);
	var dt = this._doc.createElement('dt');
